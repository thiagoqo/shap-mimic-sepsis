...
def combine_patient_data(patient_data, timestep=4, window_before=24, window_after=72):
    ...
    def get_window_measurements(measurements, start_time, end_time):
        ...
        # Define categorical columns that should NOT be averaged
        categorical_cols = [
            'oxygen_flow_device', 
            'richmond_ras', 
            'gcs', 
            'sofa_resp', 'sofa_coag', 'sofa_liver', 'sofa_cv', 'sofa_cns', 'sofa_renal', 'sofa_score',
            'sirs_score', 'septic_shock', 'sepsis'
        ]
        
        # Calculate mean for all columns first
        result = window_data.mean(axis=0, skipna=True)
        
        # Overwrite categorical columns with MAX value (conservatively assuming higher ID/Score is better to keep or simply to preserve int type)
        # Using MAX is safer than MODE for sparse windows where multiple valid values might exist.
        for col in categorical_cols:
            if col in window_data.columns:
                result[col] = window_data[col].max(skipna=True)
...
# fixgaps function removed as it contributed to temporal data leakage via linear interpolation
...
def handle_missing_values(df, missing_threshold=0.8):
    ...
    # --- TEMPORAL LEAKAGE PREVENTION FIX ---
    # Reviewer critique: "Linear interpolation and KNN imputation use future values. 
    # Consider forward-filling or using only historical data for imputation."
    # Fix: We now strict forward-fill (carry-forward) within each patient trajectory.
    cols_to_impute = cols_to_keep.tolist()
    
    # 1. Forward-fill past data for each specific patient
    df[cols_to_impute] = df.groupby('stay_id')[cols_to_impute].ffill()
    
    # 2. For remaining NaNs at the absolute beginning of a sequence (where no prior data exists),
    #    we fill with the global baseline median to prevent data loss without peeking at the future.
...
def calculate_derived_variables(df):
    ...
    def calc_news(row):
        ...
        fio2 = row['fio2']
        if not pd.isna(fio2) and fio2 > 21:
            score += 2
    ...
    df['news_score'] = df.apply(calc_news, axis=1)
    ...
    df['qsofa_score'] = df.apply(calc_qsofa, axis=1)
    
    print("Calculated qSOFA scores. Distribution:")
    print(df['qsofa_score'].value_counts().sort_index())
...
def add_sepsis_flag(df):
    ...
    1. Sepsis-3 definition:
        ...
        - SOFA score â‰¥ 2, assuming all baseline SOFA is 0
