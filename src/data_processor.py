...
class TimeSeriesDataProcessor:
    ...
    def prepare_data(self, df: pd.DataFrame) -> Tuple[np.ndarray, np.ndarray, List]:
        ...
        Returns:
            ...
            indices: List of original dataframe indices corresponding to the last timestep of each window
        """
        ...
        for _, group in grouped:
            ...
            for i in range(len(group) - self.window_size - self.prediction_horizon + 1):
                ...
                if len(window) == self.window_size:
                    ...
                    indices.append(group.index[i + self.window_size - 1])
            bar.update()
    ...
    def _prepare_mortality_data(self, df: pd.DataFrame) -> Tuple[np.ndarray, np.ndarray]:
        ...
        for _, group in grouped:
            ...
            if len(window_data) == self.window_size:  # Only use complete windows
                ...
                indices.append(group.index[self.window_size - 1]) # Index of the last timestep in the window
            bar.update()
        ...
        for _, group in grouped:
            ...
            if len(group) >= self.window_size:
                ...
                if len(window_data) == self.window_size:
                    features.append(window_data)
                    # Use the LOS value from the data and calculate remaining LOS
                    total_los = group[self.task].iloc[-1]  # Get LOS from the last row
                    # Redefine task as predicting remaining LOS at 24 hours post-admission
                    # Observation window is self.window_size * 4 hours. In days, this is self.window_size * 4 / 24.0
                    observation_days = self.window_size * 4 / 24.0
                    remaining_los = max(0.0, total_los - observation_days)
                    targets.append(remaining_los)
                    indices.append(group.index[self.window_size - 1])
            bar.update()
